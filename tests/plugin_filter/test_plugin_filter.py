...
def plugins(tmp_db):
    ...
    version_list_length = st.integers(min_value=1, max_value=15).example()
    plugin_versions = []
    ...
    # Pull versions manually and filter out invalid versions
    version_strategy = st.from_regex(Version._regex)
    max_invalid_versions = 1000
    while len(plugin_versions) < version_list_length and max_invalid_versions > 0:
        version = version_strategy.example()
        try:
            Version(version)
            plugin_versions.append(version)
        except InvalidVersion:
            max_invalid_versions -= 1

    assert (
        len(plugin_versions) == version_list_length
    ), f"Version strategy did not produce enough valid versions, got {len(plugin_versions)}, expected {version_list_length}"
...
@settings(suppress_health_check=[HealthCheck.function_scoped_fixture], deadline=1500)
